...
def yield_reduce(batches, state, steps):
    #state: is a dictionary
    #steps: are functions that take (batch, state)
    #and return a dictionary of updates to the state (or None)
...
def opt_steps(batch, state):
    if not batch: return
    return {OPTS: [sp_opt_step(**opt, mask=state["mask"]) for opt in state[OPTS]]}
...
def train_epoch(state, timer, train_batches, train_steps=default_train_steps,
                on_epoch_end=(lambda state: state)):

    return yield_reduce(train_batches, state, train_steps)

def test_epoch(state, timer, valid_batches, valid_steps=default_valid_steps,
                on_epoch_end=(lambda state: state)):

    return epoch_stats(reduce(valid_batches, state, valid_steps))
