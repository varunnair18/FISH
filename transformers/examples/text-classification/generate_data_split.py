...
def main():
    ...
    indices_list = list(range(len(train_dataset)))

    chunk_size = len(indices_list) // training_args.split
    ...
    for i in range(training_args.split):
        if i != training_args.split - 1:
            chunk = indices_list[i * chunk_size: (i + 1) * chunk_size]
        else:
            chunk = indices_list[i * chunk_size:]

        torch.save(chunk, os.path.join(training_args.output_dir, f"data_split_{i}.bin"))
