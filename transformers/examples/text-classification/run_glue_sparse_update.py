...
class ModelArguments:
    ...
    exp_name: Optional[str] = field(
        default="",
        metadata={"help": "Name of experiment. Used to save model history files."},
    )
...
def main():
    ...
    if training_args.data_split_path != "":
        indices_list = torch.load(training_args.data_split_path)
        filter_func = lambda example, indices: indices in indices_list
        train_dataset = train_dataset.filter(filter_func, with_indices=True)
    ...
    if training_args.normal_training:
        trainer = GLUETrainer(
            model=model,
            args=training_args,
    ...
    if training_args.do_train:
        ...
        with open(os.path.join(training_args.output_dir,f"eval_history_{model_args.exp_name}.json"), 'w') as fp:
            json.dump(trainer.state.log_history, fp)

        output_train_file = os.path.join(training_args.output_dir, f"train_results_{model_args.exp_name}.txt")
    ...
    torch.save(mask, os.path.join(training_args.output_dir, "mask.bin"))
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            exp_name = model_args.exp_name.replace("mnli", "mnli-mm") if task == "mnli-mm" else model_args.exp_name
            output_eval_file = os.path.join(training_args.output_dir, f"eval_results_{exp_name}.txt")
            if trainer.is_world_process_zero():
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            exp_name = model_args.exp_name.replace("mnli", "mnli-mm") if task == "mnli-mm" else model_args.exp_name
            output_file_name=f"test_results_{exp_name}"
            output_test_file = os.path.join(training_args.output_dir, f"{output_file_name}.txt")
            if trainer.is_world_process_zero():
            ...
            tsv_file_name = os.path.join(training_args.output_dir, f"{output_file_name}.tsv")
