...
def calculate_the_importance_label(model, data_loader, num_samples, cuda_device, grad_type):
    ...
    for idx, inputs in tqdm.tqdm(enumerate(data_loader), desc="Computing sample gradients..."):
        if idx >= num_samples:
            break
...
def create_mask_gradient(model, train_dataset, data_collator, num_samples, keep_ratio, sample_type, grad_type, shuffle=False):
...
def create_mask_random(model, train_dataset, data_collator, num_samples, keep_ratio):
    ...
    gradients = {}
    for name, param in model.named_parameters():
        gradients[name] = torch.rand(param.shape).to(cuda_device)
...
class SparseUpdateTrainingArguments(TrainingArguments):
    ...
    checkpoint_frequency: float = field(
        default=0,
        metadata={"help": "How often mask is recomputed in checkpointing experiments."}
    )
