...
def create_mask_random(model, train_dataset, data_collator, num_samples, keep_ratio, split=2):
    original_device = list(model.parameters())[0].device
    cuda_device = "cuda" if torch.cuda.is_available() else "cpu"
...
def main():
    ...
    if isinstance(mask, list):
        for i, m in enumerate(mask):
            torch.save(m, os.path.join(training_args.output_dir, f"mask_{i}.bin"))
