...
class SparseUpdateTrainingArguments(TrainingArguments):
    ...
    recalculate_interval: int = field(
        default=10,
        metadata={"help": "The interval to recalculate the mask after merging."}
    ) 
    evaluate_interval: int = field(
        default=3,
        metadata={"help": "The interval to evaluate model after merging."}
    ) 
...
def main():
    ...
    if training_args.mask_method == "all_ones":
        mask_method = create_all_ones_mask
    ...
    for i, mask in enumerate(masks):
        ...
        train_jobs.append(trainer.train())
    ...
    if training_args.do_train:
        ...
        for train_results in zip(*train_jobs):
            ...
            merge_models(
                models, 
                merged_model, 
                training_args.diff_aggr_method, 
                training_args.same_classifier
            )

            if step % training_args.recalculate_interval == 0:
                # re-calulate masks
                masks = mask_method(
                    merged_model,
                    train_dataset,
            ...
            if step % training_args.evaluate_interval == 0:
                tasks = [data_args.task_name]
                train_datasets = [train_dataset]

                # save the train and eval results
                global_step = trainers[0].state.global_step

                merged_trainer.model.to("cuda")
                # for train_datasets, task in zip(train_datasets, tasks):
                #     train_result = merged_trainer.evaluate(eval_dataset=train_datasets)

                #     output_train_file = os.path.join(training_args.output_dir, f"train_results_{task}_{global_step}.txt")
                #     if merged_trainer.is_world_process_zero():
                ...
                for eval_dset, task in zip(eval_datasets, tasks):
                    eval_result = merged_trainer.evaluate(eval_dataset=eval_dset)
