...
from custom_trainer import GradientResetTrainer
...
def create_mask(model, keep_ratio):
    gradients = {}
    for name, params in model.named_parameters():
        gradients[name] = torch.abs(params.grad.data)
    ...
    for k, v in gradients.items():
        ...
        if "classifier" in k:
            ...
            classifier_mask_dict[k] = torch.ones_like(v).type_as(v)
        else:
            sizes[k] = v.shape
    ...
    masks = torch.zeros_like(tensors).type_as(tensors)
...
def main():
    ...
    if model_args.model_load_path_second is not "":
        model = AutoModelForSequenceClassification.from_pretrained(
    ...
    if training_args.normal_training:
        ...
        trainer = GradientResetTrainer(
            model=model,
            args=training_args,
            keep_ratio=training_args.keep_ratio,
            train_dataset=train_dataset,
            eval_dataset=eval_dataset if training_args.do_eval else None,
            compute_metrics=compute_metrics,
            tokenizer=tokenizer,
        ...
        trainer.get_importance_mask = create_mask
