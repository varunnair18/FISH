...
def main():  
    ...
    for m, w in zip(fisher_mask.mask, trainable_params(model).values()):
        w = w.reshape(-1)
        m = m.reshape(-1)
        p = torch.where(m > 0)
        weights.append(w[p])
        p = [_p.type(torch.int32) for _p in p]
        pos.append(p)

        print(p)
        print(weights)

    torch.save({"pos": pos, "weight": weights}, "diff.torch")
    
    torch.save(list(trainable_params(model).values()), "model.torch")
